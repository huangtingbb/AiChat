...
### 代码优化
- **移除validate字段**: 从所有控制器的请求结构体中移除了validate标签字段，简化了验证逻辑
  - 更新了 `chat_controller.go` 中的创建聊天和发送消息接口
  - 更新了 `user_controller.go` 中的用户注册接口
  - 保留了其他验证相关的标签（如 `binding`, `msg_required` 等）

### 🔧 后端API增强 - 模型类型过滤
- 为`GetAvailableModelList`接口添加可选的`type`查询参数
  - 支持按模型类型过滤：`chat`, `image`, `video`, 
  - 优先按`Type`字段过滤，
  - 兼容现有调用（不传type参数返回所有模型）
- GetAllModelList服务方法，提供类型过滤功能
- 完善API文档注释，包含参数说明和枚举值
- 增加日志记录，便于调试和监控

### 🎨 前端UI增强 - 功能Tab重构
- 重新设计侧边栏功能选择，将三个功能作为同级Tab展示
  - **对话**: 聊天对话功能，配有聊天图标
  - **图片生成**: 图片生成功能，配有图片图标（UI已完成，逻辑待实现）
  - **视频生成**: 视频生成功能，配有视频图标（UI已完成，逻辑待实现）
- 新建按钮文字根据当前选中的Tab动态变化（新建对话/新建图片/新建视频）
- 采用图标+文字的垂直布局，提升用户体验
- Tab切换采用现代化设计，与整体UI风格保持一致
- 支持流畅的动画过渡效果
